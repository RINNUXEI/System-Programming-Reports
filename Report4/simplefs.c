...
static int simple_rename (const char * old_path, const char * new_path)
{
  struct directory_entry *p;
  if((p = search_file(old_path)) == NULL){
    return -ENOENT;
  }
  if(search_file(new_path)){
    return -EEXIST;
  }
  strcpy(p->name, new_path + 1);
  return 0;
}
...
static int simple_truncate (const char * path, off_t length)
{
  ...
  if(p->file->length < length){
    new_data = calloc(length, 1);
    if(new_data == NULL)
      return -ENOSPC;
    memcpy(new_data, p->file->data, p->file->length);
    free(p->file->data);
    p->file->data = new_data;
